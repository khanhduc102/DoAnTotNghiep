...
const notFoundHandler = (req, res, next) => {
  next(ApiError.notFound(`Khong tim thay endpoint ${req.method} ${req.originalUrl}`));
};
...
const errorHandler = (err, req, res, next) => {
  ...
  if (err.code === 'P2002') {
    statusCode = 409;
    const field = err.meta?.target?.[0] || 'Gia tri';
    message = `${field} da ton tai trong he thong`;
  } else if (err.code === 'P2025') {
    statusCode = 404;
    message = 'Khong tim thay ban ghi can thao tac';
  } else if (err.code === 'P2003') {
    statusCode = 400;
    message = 'Du lieu tham chieu khong hop le';
  }
